...
function MiniBarChart({ scores, height = 100, width = 280 }) {
  ...
  return <svg width={width} height={height + 20} style={{ display:'block' }}>
    ...
    {scores.map((s, i) => {
      const h = (s / 100) * height, x = 10 + i * (barW + 4), color = s >= 60 ? C.green : C.red
      return <g key={i}><rect x={x} y={height - h} width={barW} height={h} rx={3} fill={color} opacity={0.8} /><text x={x + barW / 2} y={height - h - 4} fill={C.t1} fontSize={10} textAnchor="middle" fontWeight={700}>{s}</text><text x={x + barW / 2} y={height + 14} fill={C.t2} fontSize={9} textAnchor="middle">#{i + 1}</text></g>
    })}
  </svg>
}
...
function AdminLogin({ onLogin, onBack }) {
  ...
  const login = async () => { setLoading(true); setErr(''); const { data } = await supabase.from('admin_config').select('value').eq('key','admin_password').single(); if (data && data.value === pw) onLogin(); else { setErr('비밀번호가 틀렸습니다.'); setLoading(false) } }
...
function AdminDashboard({ onBack }) {
  ...
  useEffect(() => { if (!exam) return; const ch = supabase.channel('admin-rt').on('postgres_changes', { event: '*', schema: 'public', table: 'students', filter: `exam_id=eq.${exam.id}` }, () => loadStudents()).subscribe(); return () => supabase.removeChannel(ch) }, [exam?.id])

  const loadActiveExam = async () => { const { data } = await supabase.from('exams').select('*').eq('is_active', true).order('created_at', { ascending: false }).limit(1); if (data?.length) { setExam(data[0]); setTitle(data[0].title); setTimeLimit(data[0].time_limit_min); await loadQuestions(data[0].id); await loadStudents(data[0].id) } else { const { data: r } = await supabase.from('exams').select('*').order('created_at', { ascending: false }).limit(1); if (r?.length) { setExam(r[0]); setTitle(r[0].title); setTimeLimit(r[0].time_limit_min); await loadQuestions(r[0].id); await loadStudents(r[0].id) } } }
  ...
  const createExam = async () => { await supabase.from('exams').update({ is_active: false }).eq('is_active', true); const { data } = await supabase.from('exams').insert({ title, time_limit_min: timeLimit, is_active: false }).select().single(); if (data) { setExam(data); addLog(`시험 생성: "${title}"`); setQuestions([]); setStudents([]) } }
  const startExam = async () => { if (!exam) return; const incQ = questions.filter(q => q.is_included !== false).length; if (incQ === 0) { alert('포함된 문제가 없습니다!'); return }; await supabase.from('exams').update({ is_active: true, started_at: new Date().toISOString() }).eq('id', exam.id); setExam({ ...exam, is_active: true }); addLog(`═══ 시험 시작 (${incQ}문제, ${timeLimit}분) ═══`) }
  ...
  // Roster
  const uploadRoster = async (e) => { const file = e.target.files?.[0]; if (!file) return; const text = await file.text(); const entries = text.split('\n').map(l => l.trim()).filter(Boolean).map(l => { const p = l.split(/[,\t]/).map(s => s.trim()).filter(Boolean); return p.length >= 2 ? { student_id: p[0], name: p[1] } : null }).filter(Boolean); if (!entries.length) { alert('파싱된 학생이 없습니다.'); return }; const { error } = await supabase.from('roster').upsert(entries, { onConflict: 'student_id' }); if (error) addLog('오류: ' + error.message); else { addLog(`명단 ${entries.length}명 업로드`); loadRoster() }; if (rosterFileRef.current) rosterFileRef.current.value = '' }
  const addRosterStudent = async () => { if (!newSid.trim() || !newName.trim()) { alert('학번과 이름을 입력하세요'); return }; const { error } = await supabase.from('roster').upsert({ student_id: newSid.trim(), name: newName.trim() }, { onConflict: 'student_id' }); if (error) alert('오류: ' + error.message); else { setNewSid(''); setNewName(''); loadRoster(); addLog(`학생 추가: ${newSid} ${newName}`) } }
  const deleteSelectedRoster = async () => { if (selR.size === 0 || !confirm(`${selR.size}명을 삭제하시겠습니까?`)) return; await supabase.from('roster').delete().in('id', Array.from(selR)); setSelR(new Set()); loadRoster(); addLog(`${selR.size}명 삭제`) }
  ...
  // Questions
  const handleQFile = async (e) => { const file = e.target.files?.[0]; if (!file || !exam) return; try { const text = await file.text(); const ext = file.name.split('.').pop().toLowerCase(); let parsed; if (ext === 'json') { parsed = JSON.parse(text); if (!Array.isArray(parsed)) throw new Error('JSON은 배열') } else parsed = parseTXT(text); if (!parsed.length) { alert('파싱된 문제가 없습니다'); return }; const rows = parsed.map((q, i) => ({ exam_id: exam.id, type: q.type, category: q.category || '일반', question: q.question, options: q.options || [], answer: q.type === 'multiple_choice' ? q.answer : q.type === 'ox' ? q.answer : q.type === 'short_answer' ? q.answer : q.pairs || [], pairs: q.pairs || [], explanation: q.explanation || '', sort_order: i + 1 + questions.length, is_included: true })); const { error } = await supabase.from('questions').insert(rows); if (error) { addLog('오류: ' + error.message); return }; addLog(`문제 ${parsed.length}개 업로드`); await loadQuestions() } catch (err) { alert('파일 오류: ' + err.message) }; if (fileRef.current) fileRef.current.value = '' }
  ...
  const deleteSelectedQ = async () => { if (selQ.size === 0 || !confirm(`${selQ.size}문항 삭제?`)) return; await supabase.from('questions').delete().in('id', Array.from(selQ)); setSelQ(new Set()); loadQuestions(); addLog(`${selQ.size}문항 삭제`) }
  const toggleInclude = async (id, cur) => { await supabase.from('questions').update({ is_included: !cur }).eq('id', id); loadQuestions() }
  const bulkToggleInclude = async (inc) => { if (selQ.size === 0) return; await supabase.from('questions').update({ is_included: inc }).in('id', Array.from(selQ)); loadQuestions(); addLog(`${selQ.size}문항 ${inc?'포함':'제외'}`) }
  const saveEditQ = async () => { if (!editQ) return; await supabase.from('questions').update({ question: editQ.question, category: editQ.category, explanation: editQ.explanation, options: editQ.options, answer: editQ.answer, pairs: editQ.pairs }).eq('id', editQ.id); setEditQ(null); loadQuestions(); addLog(`문항 수정`) }
  ...
  return <div style={S.page}><div style={S.container}>
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16, flexWrap:'wrap', gap:10 }}>
      <div><h1 style={{ color:C.t1, fontSize:20, fontWeight:800, margin:0 }}>🔧 관리자 대시보드</h1>
        <p style={{ color:C.t2, fontSize:12, margin:'4px 0 0' }}>{exam ? `"${exam.title}" | 문제: ${incCount}/${questions.length} | 접속: ${students.length}명 | 명단: ${roster.length}명` : '시험을 생성하세요'}{exam?.is_active && <span style={{ color:C.green, marginLeft:8 }}>● 진행중</span>}</p></div>
      <button onClick={onBack} style={S.btnSec}>← 나가기</button>
    </div>
    <div style={S.card}><div style={{ display:'flex', gap:10, alignItems:'flex-end', flexWrap:'wrap' }}>
    ...
    {tab === 'roster' && <>
      <div style={{ ...S.card, display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <button onClick={() => rosterFileRef.current?.click()} style={S.btn(C.purple)}>📄 명단 업로드</button>
        <span style={{ color:C.t2, fontSize:11 }}>학번,이름 또는 학번[탭]이름</span>
        {selR.size > 0 && <button onClick={deleteSelectedRoster} style={{ ...S.btnSm(C.red), marginLeft:'auto' }}>🗑 삭제 ({selR.size})</button>}
      </div>
      ...
      <div style={S.card}><h3 style={{ color:C.t1, fontSize:15, fontWeight:700, margin:'0 0 12px' }}>등록 명단 ({roster.length}명)</h3>
        ...
        <div style={{ overflowX:'auto' }}><table style={{ width:'100%', borderCollapse:'collapse' }}><thead><tr>
          ...
          <th style={S.th}>학번</th><th style={S.th}>이름</th><th style={S.th}>응시</th><th style={S.th}>최근점수</th><th style={S.th}>추이</th><th style={S.th}></th>
        </tr></thead><tbody>{roster.map(r => {
          const scores = examHistory.filter(h => h.student_id===r.student_id && h.score!=null).map(h => h.score).reverse()
          return <tr key={r.id} style={{ background: selR.has(r.id)?'rgba(59,130,246,0.06)':'transparent' }}>
            ...
            <td style={S.td}>{r.student_id}</td><td style={S.td}>{r.name}</td>
            <td style={S.td}>{scores.length}회</td>
            <td style={{ ...S.td, fontWeight:700, color:scores.length?scores[scores.length-1]>=60?C.green:C.red:C.t2 }}>{scores.length?`${scores[scores.length-1]}점`:'-'}</td>
            <td style={S.td}><MiniBarChart scores={scores} height={36} width={Math.max(60, scores.length*28+20)} /></td>
            <td style={S.td}><button onClick={() => setDetailStudent(r)} style={S.btnSm(C.blue)}>📊</button></td>
          </tr>
        })}</tbody></table></div>}
      </div>
      {detailStudent && <div style={S.modal} onClick={() => setDetailStudent(null)}><div style={{ ...S.card, width:520, maxHeight:'80vh', overflowY:'auto', margin:20 }} onClick={e => e.stopPropagation()}>
        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:16 }}><h3 style={{ color:C.t1, fontSize:18, fontWeight:700, margin:0 }}>📊 {detailStudent.name} ({detailStudent.student_id})</h3><button onClick={() => setDetailStudent(null)} style={{ background:'none', border:'none', color:C.t2, fontSize:18, cursor:'pointer' }}>✕</button></div>
        {(() => { const scores = examHistory.filter(h => h.student_id===detailStudent.student_id && h.score!=null).map(h => h.score).reverse(); const hist = examHistory.filter(h => h.student_id===detailStudent.student_id && h.session_id); return <>
          <p style={{ color:C.t2, fontSize:13, margin:'0 0 8px' }}>응시: <strong style={{ color:C.t1 }}>{scores.length}회</strong> | 평균: <strong style={{ color:(scores.reduce((a,b)=>a+b,0)/scores.length||0)>=60?C.green:C.red }}>{scores.length?(scores.reduce((a,b)=>a+b,0)/scores.length).toFixed(1):'-'}점</strong></p>
    ...
    {/* ═══ 응시 현황 ═══ */}
    {tab === 'students' && <div style={S.card}><h3 style={{ color:C.t1, fontSize:15, fontWeight:700, margin:'0 0 12px' }}>현재 시험 ({students.length}명)</h3>
    ...
    {tab === 'questions' && <>
      <div style={{ ...S.card, display:'flex', gap:6, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <button onClick={() => fileRef.current?.click()} disabled={!exam} style={S.btn(C.purple)}>📄 업로드</button>
        {selQ.size > 0 && <><button onClick={deleteSelectedQ} style={S.btnSm(C.red)}>🗑 삭제({selQ.size})</button><button onClick={() => bulkToggleInclude(true)} style={S.btnSm(C.green)}>✅ 포함</button><button onClick={() => bulkToggleInclude(false)} style={S.btnSm(C.yellow)}>⛔ 제외</button></>}
        <span style={{ color:C.t2, fontSize:11, marginLeft:'auto' }}>포함: {incCount} / 전체: {questions.length}</span>
      </div>
      <div style={S.card}>{questions.length === 0 ? <p style={{ color:C.t2 }}>문제를 업로드하세요.</p> :
        <div style={{ overflowX:'auto' }}><table style={{ width:'100%', borderCollapse:'collapse' }}><thead><tr>
          ...
          <th style={S.th}>#</th><th style={S.th}>상태</th><th style={S.th}>유형</th><th style={S.th}>카테고리</th><th style={S.th}>문제</th><th style={S.th}></th>
        </tr></thead><tbody>{questions.map((q, i) => { const inc = q.is_included !== false; return <tr key={q.id} style={{ opacity:inc?1:0.45, background:selQ.has(q.id)?'rgba(59,130,246,0.06)':'transparent' }}>
          <td style={S.td}><input type="checkbox" checked={selQ.has(q.id)} onChange={() => toggleQ(q.id)} style={S.checkbox} /></td>
          <td style={S.td}>{i+1}</td>
          <td style={S.td}><button onClick={() => toggleInclude(q.id, inc)} style={{ ...S.btnSm(inc?C.green:C.red), padding:'3px 8px', fontSize:11 }}>{inc?'포함':'제외'}</button></td>
      ...
      {editQ && <div style={S.modal} onClick={() => setEditQ(null)}><div style={{ ...S.card, width:560, maxHeight:'85vh', overflowY:'auto', margin:20 }} onClick={e => e.stopPropagation()}>
        <h3 style={{ color:C.t1, fontSize:17, fontWeight:700, margin:'0 0 16px' }}>✏️ 문제 수정</h3>
        <div style={{ marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>카테고리</label><input value={editQ.category} onChange={e => setEditQ({...editQ, category:e.target.value})} style={S.input} /></div>
        <div style={{ marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>문제</label><textarea value={editQ.question} onChange={e => setEditQ({...editQ, question:e.target.value})} rows={3} style={{ ...S.input, resize:'vertical' }} /></div>
        {editQ.type === 'multiple_choice' && <div style={{ marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>보기</label>{(editQ.options||[]).map((opt, i) => <div key={i} style={{ display:'flex', gap:8, marginBottom:6, alignItems:'center' }}><span style={{ color:editQ.answer===i?C.green:C.t2, fontWeight:700, fontSize:12, width:20 }}>{i+1})</span><input value={opt} onChange={e => { const o=[...(editQ.options||[])]; o[i]=e.target.value; setEditQ({...editQ, options:o}) }} style={{ ...S.input, flex:1 }} /><input type="radio" name="mc_ans" checked={editQ.answer===i} onChange={() => setEditQ({...editQ, answer:i})} style={{ accentColor:C.green }} /></div>)}</div>}
        {editQ.type === 'ox' && <div style={{ marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>정답</label><div style={{ display:'flex', gap:12 }}>{[{l:'O',v:true},{l:'X',v:false}].map(o => <label key={o.l} style={{ display:'flex', alignItems:'center', gap:6, color:C.t1, cursor:'pointer' }}><input type="radio" name="ox_ans" checked={editQ.answer===o.v} onChange={() => setEditQ({...editQ, answer:o.v})} style={{ accentColor:C.green }} /> {o.l}</label>)}</div></div>}
        {editQ.type === 'short_answer' && <div style={{ marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>정답 (쉼표 구분)</label><input value={Array.isArray(editQ.answer)?editQ.answer.join(', '):editQ.answer} onChange={e => setEditQ({...editQ, answer:e.target.value.split(',').map(s=>s.trim()).filter(Boolean)})} style={S.input} /></div>}
        {editQ.type === 'matching' && <div style={{ marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>매칭 쌍</label>{(editQ.pairs||[]).map((p, i) => <div key={i} style={{ display:'flex', gap:8, marginBottom:6 }}><input value={p.left||''} onChange={e => { const pp=[...(editQ.pairs||[])]; pp[i]={...pp[i], left:e.target.value}; setEditQ({...editQ, pairs:pp}) }} placeholder="왼쪽" style={{ ...S.input, flex:1 }} /><span style={{ color:C.blue, fontWeight:700, alignSelf:'center' }}>=</span><input value={p.right||''} onChange={e => { const pp=[...(editQ.pairs||[])]; pp[i]={...pp[i], right:e.target.value}; setEditQ({...editQ, pairs:pp}) }} placeholder="오른쪽" style={{ ...S.input, flex:1 }} /></div>)}</div>}
        <div style={{ marginBottom:16 }}><label style={{ fontSize:12, color:C.t2, display:'block', marginBottom:4 }}>해설</label><textarea value={editQ.explanation} onChange={e => setEditQ({...editQ, explanation:e.target.value})} rows={2} style={{ ...S.input, resize:'vertical' }} /></div>
        <div style={{ display:'flex', gap:8 }}><button onClick={saveEditQ} style={S.btn(C.green)}>💾 저장</button><button onClick={() => setEditQ(null)} style={S.btnSec}>취소</button></div>
      </div></div>}
    </>}

    {/* ═══ 문제 분석 ═══ */}
    {tab === 'analytics' && <div style={S.card}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}><h3 style={{ color:C.t1, fontSize:15, fontWeight:700, margin:0 }}>문제별 분석</h3><button onClick={loadAnalytics} style={S.btnSm(C.blue)}>🔄 새로고침</button></div>
      {analytics.length === 0 ? <p style={{ color:C.t2 }}>데이터가 없습니다.</p> :
      <div style={{ overflowX:'auto' }}><table style={{ width:'100%', borderCollapse:'collapse' }}><thead><tr>{['#','상태','유형','카테고리','문제','응시','정답률','평균(초)','난이도'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{analytics.map(a => <tr key={a.question_id} style={{ opacity:a.is_included!==false?1:0.4 }}>
        <td style={S.td}>{a.sort_order}</td><td style={S.td}><span style={S.tag(a.is_included!==false?C.green:C.red)}>{a.is_included!==false?'포함':'제외'}</span></td><td style={S.td}><span style={S.tag(TYPE_C[a.type]||C.t2)}>{TYPE_L[a.type]}</span></td><td style={S.td}>{a.category}</td><td style={{ ...S.td, maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{a.question}</td><td style={S.td}>{a.attempt_count}</td><td style={{ ...S.td, fontWeight:700, color:a.correct_rate>=60?C.green:a.correct_rate>0?C.red:C.t2 }}>{a.correct_rate}%</td><td style={S.td}>{a.avg_time_sec}초</td><td style={S.td}><span style={S.tag(a.difficulty==='어려움'?C.red:a.difficulty==='보통'?C.yellow:a.difficulty==='쉬움'?C.green:C.t2)}>{a.difficulty}</span></td>
      </tr>)}</tbody></table></div>}
    </div>}
...
function StudentLogin({ onLogin, onBack }) {
  const [sid, setSid] = useState(''), [name, setName] = useState(''), [save, setSave] = useState(false), [err, setErr] = useState(''), [loading, setLoading] = useState(false)
  ...
  const login = async () => {
    ...
    if (!rm) { setErr('등록되지 않은 학번입니다.'); setLoading(false); return }
    if (rm.name !== name.trim()) { setErr('학번과 이름이 일치하지 않습니다.'); setLoading(false); return }
  ...
  return <div style={{ ...S.page, display:'flex', alignItems:'center', justifyContent:'center' }}><div style={{ ...S.card, width:400, textAlign:'center' }}>
    ...
    <div style={{ textAlign:'left', marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, marginBottom:4, display:'block' }}>학번</label><input value={sid} onChange={e => setSid(e.target.value)} placeholder="예: 20241234" style={S.input} /></div>
    <div style={{ textAlign:'left', marginBottom:12 }}><label style={{ fontSize:12, color:C.t2, marginBottom:4, display:'block' }}>이름</label><input value={name} onChange={e => setName(e.target.value)} placeholder="예: 홍길동" onKeyDown={e => e.key==='Enter' && login()} style={S.input} /></div>
    <label style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16, cursor:'pointer', justifyContent:'flex-start' }}><input type="checkbox" checked={save} onChange={e => setSave(e.target.checked)} style={S.checkbox} /><span style={{ fontSize:13, color:C.t2 }}>학번과 이름 저장 (다음 접속 시 자동 입력)</span></label>
    {err && <p style={{ color:C.red, fontSize:13, marginBottom:12 }}>{err}</p>}
...
function StudentLobby({ studentId, studentName, onStartExam, onBack }) {
  ...
  const [lobbyTab, setLobbyTab] = useState('info')
  ...
  const pollExam = async () => {
    ...
    if (data?.length) {
      setExam(data[0]); setExamReady(true)
      const { data: qs } = await supabase.from('questions').select('id', { count: 'exact' }).eq('exam_id', data[0].id).eq('is_included', true)
      setQCount(qs?.length || 0)
    } else {
      const { data: recent } = await supabase.from('exams').select('*').order('created_at', { ascending: false }).limit(1)
      if (recent?.length) { setExam(recent[0]); setExamReady(false); const { data: qs } = await supabase.from('questions').select('id', { count: 'exact' }).eq('exam_id', recent[0].id).eq('is_included', true); setQCount(qs?.length || 0) }
      else { setExam(null); setExamReady(false) }
    }
  }

  const loadPastResults = async () => {
    const { data: sessions } = await supabase.from('students').select('*, exams(title)').eq('student_id', studentId).order('created_at', { ascending: false })
  ...
  const loadWrongQuestions = async () => {
    ...
    // 2. 오답 기록 조회
    const { data: wrongAns } = await supabase.from('answers').select('question_id, is_correct, questions(question, type, category, explanation)').in('student_id', sessionIds)
    if (!wrongAns?.length) { setWrongQuestions([]); return }
    // 3. 문제별 오답 횟수 집계
    const qMap = {}
    wrongAns.forEach(a => {
      const qid = a.question_id
      if (!qMap[qid]) qMap[qid] = { ...a.questions, question_id: qid, wrongCount: 0, totalCount: 0 }
      qMap[qid].totalCount++
      if (!a.is_correct) qMap[qid].wrongCount++
    })
    // 오답 1회 이상인 문제만, 오답 많은 순
    const sorted = Object.values(qMap).filter(q => q.wrongCount > 0).sort((a, b) => b.wrongCount - a.wrongCount)
  ...
  const handleStart = async () => {
    ...
    const { data: student, error } = await supabase.from('students').upsert({
      student_id: studentId, name: studentName, exam_id: exam.id,
      status: '응시중', started_at: new Date().toISOString(),
      total_questions: qCount, current_question: 1
  ...
  return <div style={S.page}><div style={{ ...S.container, maxWidth:700 }}>
    ...
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        ...
        <div><h2 style={{ color:C.t1, fontSize:18, fontWeight:700, margin:0 }}>{studentName}</h2><p style={{ color:C.t2, fontSize:12, margin:0 }}>학번: {studentId}</p></div>
      </div>
    ...
    <div style={{ ...S.card, textAlign:'center', padding:32, border: examReady ? '1px solid rgba(16,185,129,0.2)' : '1px solid rgba(148,163,184,0.1)' }}>
      {exam ? <>
        <h3 style={{ color:C.t1, fontSize:20, fontWeight:700, margin:'0 0 8px' }}>{exam.title}</h3>
        <div style={{ display:'flex', justifyContent:'center', gap:24, marginBottom:20 }}>
          <div><span style={{ color:C.t2, fontSize:12 }}>문항 수</span><p style={{ color:C.t1, fontSize:22, fontWeight:800, margin:'4px 0 0' }}>{qCount}문제</p></div>
          <div><span style={{ color:C.t2, fontSize:12 }}>제한 시간</span><p style={{ color:C.t1, fontSize:22, fontWeight:800, margin:'4px 0 0' }}>{exam.time_limit_min}분</p></div>
        </div>
        {examReady ? (
          <button onClick={handleStart} style={{ ...S.btn(C.green), padding:'16px 48px', fontSize:18, borderRadius:14, boxShadow:'0 4px 20px rgba(16,185,129,0.3)' }}>
            ▶ 시험 응시 시작
          </button>
        ) : (
          <div>
            <div style={{ fontSize:36, marginBottom:8 }}>⏳</div>
            <p style={{ color:C.yellow, fontSize:15, fontWeight:600, margin:'0 0 4px' }}>시험 대기 중</p>
            <p style={{ color:C.t2, fontSize:13, margin:0 }}>관리자가 시험을 시작하면 버튼이 활성화됩니다</p>
          </div>
        )}
    ...
    {/* 탭 */}
    <div style={{ display:'flex', borderBottom:'1px solid rgba(148,163,184,0.1)', marginBottom:14 }}>
      <button onClick={() => setLobbyTab('info')} style={tabS('info')}>📊 나의 성적</button>
      <button onClick={() => setLobbyTab('wrong')} style={tabS('wrong')}>❌ 자주 틀리는 문제</button>
    </div>

    {/* 나의 성적 */}
    {lobbyTab === 'info' && <>
      {pastScores.length > 0 && <div style={S.card}>
        <h3 style={{ color:C.t1, fontSize:15, fontWeight:700, margin:'0 0 12px' }}>점수 추이</h3>
        <MiniBarChart scores={pastScores} height={100} width={Math.min(600, Math.max(200, pastScores.length * 50 + 30))} />
        <p style={{ color:C.t2, fontSize:12, marginTop:8 }}>
          평균: <strong style={{ color:C.t1 }}>{(pastScores.reduce((a,b)=>a+b,0)/pastScores.length).toFixed(1)}점</strong>
          {' '} | 최고: <strong style={{ color:C.green }}>{Math.max(...pastScores)}점</strong>
          {' '} | 최저: <strong style={{ color:C.red }}>{Math.min(...pastScores)}점</strong>
        </p>
      </div>}
      <div style={S.card}>
        <h3 style={{ color:C.t1, fontSize:15, fontWeight:700, margin:'0 0 12px' }}>응시 이력 ({pastResults.length}회)</h3>
        {pastResults.length === 0 ? <p style={{ color:C.t2, fontSize:13 }}>아직 응시 이력이 없습니다. 첫 시험을 응시해보세요!</p> :
        <div style={{ overflowX:'auto' }}><table style={{ width:'100%', borderCollapse:'collapse' }}><thead><tr>{['시험명','상태','점수','정답','합격','제출 시간'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{pastResults.map((r, i) => {
          ...
          return <tr key={i}>
            ...
            <td style={S.td}>{r.submitted_at?new Date(r.submitted_at).toLocaleString('ko-KR'):'-'}</td>
          </tr>
        })}</tbody></table></div>}
      </div>
    </>}
    ...
    {/* 자주 틀리는 문제 */}
    {lobbyTab === 'wrong' && <div style={S.card}>
      <h3 style={{ color:C.t1, fontSize:15, fontWeight:700, margin:'0 0 4px' }}>자주 틀리는 문제</h3>
      <p style={{ color:C.t2, fontSize:12, margin:'0 0 16px' }}>과거 응시에서 오답이 많았던 문제 순으로 표시됩니다.</p>
      {wrongQuestions.length === 0 ? <p style={{ color:C.t2, fontSize:13 }}>오답 기록이 없습니다. 시험을 응시하면 분석 결과가 여기에 표시됩니다.</p> :
      <div>
        {wrongQuestions.map((wq, i) => {
          const wrongRate = Math.round(wq.wrongCount / wq.totalCount * 100)
          return <div key={wq.question_id} style={{ padding:14, borderRadius:12, marginBottom:10, background: wrongRate >= 80 ? 'rgba(239,68,68,0.06)' : wrongRate >= 50 ? 'rgba(245,158,11,0.06)' : 'rgba(148,163,184,0.04)', border: `1px solid ${wrongRate >= 80 ? 'rgba(239,68,68,0.15)' : wrongRate >= 50 ? 'rgba(245,158,11,0.12)' : 'rgba(148,163,184,0.08)'}` }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, flexWrap:'wrap' }}>
              <span style={{ fontSize:14, fontWeight:800, color: wrongRate >= 80 ? C.red : wrongRate >= 50 ? C.yellow : C.t2 }}>#{i + 1}</span>
              <span style={S.tag(TYPE_C[wq.type] || C.t2)}>{TYPE_L[wq.type] || wq.type}</span>
              <span style={S.tag(C.t2)}>{wq.category}</span>
              <span style={{ marginLeft:'auto', fontSize:12, fontWeight:700, color: wrongRate >= 80 ? C.red : wrongRate >= 50 ? C.yellow : C.t2 }}>
                오답 {wq.wrongCount}/{wq.totalCount}회 ({wrongRate}%)
              </span>
            </div>
            {/* 오답률 바 */}
            <div style={{ height:4, background:'rgba(148,163,184,0.1)', borderRadius:2, marginBottom:10 }}>
              <div style={{ height:'100%', width:`${wrongRate}%`, background: wrongRate >= 80 ? C.red : wrongRate >= 50 ? C.yellow : C.t2, borderRadius:2 }} />
            </div>
            <p style={{ color:C.t1, fontSize:14, fontWeight:600, lineHeight:1.6, margin:'0 0 6px' }}>{wq.question}</p>
            {wq.explanation && <p style={{ color:C.t2, fontSize:12, lineHeight:1.6, margin:0 }}>💡 {wq.explanation}</p>}
          </div>
        })}
      </div>}
    </div>}
...
function StudentExam({ student: initStudent, exam: initExam, onFinish, onAbandon }) {
  ...
  const submitExam = async () => {
    ...
    await supabase.from('students').update({ status:'제출완료', submitted_at: new Date().toISOString(), score, correct_count: cc, total_questions: questions.length }).eq('id', student.id)
    onFinish({ questions, answers, timeSpent: timeSpent.current, score, correctCount: cc })
  }
  ...
  return <div style={S.page}><div style={S.container}>
    ...
    <div style={S.card}>
      ...
      {q.type==='multiple_choice'&&<MCQ q={q} value={answers[q.id]} onChange={v => setAnswers({...answers,[q.id]:v})} />}
      {q.type==='ox'&&<OXQ q={q} value={answers[q.id]} onChange={v => setAnswers({...answers,[q.id]:v})} />}
      {q.type==='short_answer'&&<SAQ q={q} value={answers[q.id]} onChange={v => setAnswers({...answers,[q.id]:v})} />}
      {q.type==='matching'&&<MatchQ q={q} value={answers[q.id]} onChange={v => setAnswers({...answers,[q.id]:v})} />}
    </div>
    <div style={{ display:'flex', justifyContent:'space-between', marginTop:8, flexWrap:'wrap', gap:8 }}>
      ...
      {ci<questions.length-1?<button onClick={() => navigateTo(ci+1)} style={S.btn()}>다음 ▶</button>:<button onClick={() => { if(confirm(`제출하시겠습니까?\n작성: ${ac}/${questions.length}`)) submitExam() }} style={S.btn(C.green)}>✔ 제출</button>}
    </div>
  </div></div>
}

/* ─── Question Components ─── */
function MCQ({ q, value, onChange }) { const lb=['A','B','C','D']; return <div style={{ display:'flex', flexDirection:'column', gap:8 }}>{(q.options||[]).map((opt,i) => { const s=value===i; return <button key={i} onClick={() => onChange(i)} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:10, background:s?'rgba(59,130,246,0.12)':'rgba(148,163,184,0.05)', border:s?'1px solid rgba(59,130,246,0.4)':'1px solid rgba(148,163,184,0.1)', color:C.t1, fontSize:14, cursor:'pointer', textAlign:'left' }}><span style={{ width:28, height:28, borderRadius:6, background:s?'rgba(59,130,246,0.25)':'rgba(148,163,184,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:12, color:s?C.blue:C.t2, flexShrink:0 }}>{lb[i]}</span>{opt}</button> })}</div> }
...
function StudentResult({ data, onHome }) {
  ...
  return <div style={S.page}><div style={S.container}>
    <div style={{ ...S.card, textAlign:'center', border:`2px solid ${mc}33`, padding:32 }}>
      <div style={{ width:100, height:100, borderRadius:'50%', margin:'0 auto 14px', background:`${mc}15`, border:`3px solid ${mc}44`, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column' }}><span style={{ fontSize:32, fontWeight:800, color:mc }}>{score}</span><span style={{ fontSize:12, color:C.t2 }}>점</span></div>
...
export default function App() {
  ...
  const [loginData, setLoginData] = useState(null)   // { studentId, studentName }
  const [examData, setExamData] = useState(null)      // { student, exam }
